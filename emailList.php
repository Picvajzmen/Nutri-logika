...
<body>
    ...
    ======================= -->
        ...
        <?php if (isset($_GET['delEmail_id'])) {

            $id = $_GET['delEmail_id'];
            $sql = "DELETE FROM email WHERE id = $id";
            $query = $conn->prepare($sql);
            $query->execute();
            header("Location:emailList.php?delete=true");
        }  ?>
        <!-- =====================
     SELECT ALL
    ======================= -->
        ...
        $query = $conn->prepare($sql);
        $query->execute();
        $allEmail = $query->get_result()->fetch_all(MYSQLI_ASSOC);
        ?>
        <div class="dashboard-title__wrapper">
            <h1>Spisak prijavljenih za PDF</h1>
        </div>
                    ...
                    <li class="person">
                        <div class="person__info">
                            <ul class="person__name">
                                ...
                                <li class="info-item"><span>Email:</span> <?php echo ($email['email']); ?></li>
